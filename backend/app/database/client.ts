...
export async function getDb(): Promise<DbInstanceWrapper> {
  ...
  // Helper to match a document against a MongoDB-like query in local fallback
    ...
    for (const key in query) {
      ...
      if (key === "$or") {
        if (Array.isArray(queryVal)) {
          const anyMatch = queryVal.some(subQuery => matchQuery(item, subQuery));
      ...
      if (key === "$and") {
        if (Array.isArray(queryVal)) {
          const allMatch = queryVal.every(subQuery => matchQuery(item, subQuery));
      ...
      if (queryVal && typeof queryVal === "object" && !Array.isArray(queryVal)) {
        if ("$in" in queryVal) {
          const inList = queryVal["$in"];
          if (Array.isArray(inList)) {
            if (!inList.includes(itemVal)) return false;
          }
        } else if ("$nin" in queryVal) {
          const ninList = queryVal["$nin"];
          if (Array.isArray(ninList)) {
            if (ninList.includes(itemVal)) return false;
          }
        } else {
          // Direct object/nested structure comparison fallback
  ...
  // Helper to apply updates to a document using MongoDB-like operators
  const applyUpdate = (item: any, update: any): any => {
    const updated = { ...item };
    ...
    if (update.$addToSet) {
      for (const key in update.$addToSet) {
        ...
        if (typeof val === "object" && val !== null && "$each" in val) {
          const eachList = (val as any).$each;
          if (Array.isArray(eachList)) {
            eachList.forEach((el: any) => {
              if (!updated[key].includes(el)) {
                updated[key].push(el);
              }
            });
          }
      ...
      for (const key in update.$pull) {
        if (Array.isArray(updated[key])) {
          ...
          if (typeof val === "object" && val !== null && "$in" in val) {
            ...
            if (Array.isArray(inList)) {
              updated[key] = updated[key].filter((el: any) => !inList.includes(el));
            }
          } else {
            updated[key] = updated[key].filter((el: any) => el !== val);
          }
        }
      }
    }
    ...
    collection: (name: string): DbCollectionWrapper => {
      return {
        find: async (query: any = {}) => {
          ...
          return list.filter((item: any) => matchQuery(item, query));
        },
        findOne: async (query: any) => {
          const data = readLocalDb();
          const list = data[name] || [];
        ...
        updateOne: async (query: any, update: any) => {
          ...
          list.forEach((item: any, idx: number) => {
            if (matchQuery(item, query)) {
              list[idx] = applyUpdate(item, update);
              modifiedCount++;
            }
          });
          data[name] = list;
        ...
        deleteOne: async (query: any) => {
          ...
          const remaining = list.filter((item: any) => !matchQuery(item, query));
          data[name] = remaining;
          writeLocalDb(data);
        ...
        countDocuments: async (query: any = {}) => {
          const data = readLocalDb();
          const list = data[name] || [];
          const filtered = list.filter((item: any) => matchQuery(item, query));
